...
export default function AdminWrapper({ children, adminName }: { children: React.ReactNode, adminName: string }) {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans selection:bg-brand-gold selection:text-gray-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 text-white flex flex-col transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 hide-scrollbar">
          ...
          <Link href="/map" target="_blank" className="flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 font-bold text-sm text-gray-400 hover:text-emerald-400 hover:bg-gray-800 group">
        ...
        <div className="p-4 border-t border-gray-800 shrink-0">
          <Link href="/admin/logout" className="flex items-center gap-3 justify-center w-full py-3 rounded-xl text-red-400 hover:text-white hover:bg-red-500/20 hover:border-red-500/50 border border-transparent transition-all text-sm font-bold">
