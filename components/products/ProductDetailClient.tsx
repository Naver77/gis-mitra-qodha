...
export default function ProductDetailClient({ product }: { product: Product }) {
  ...
  const handleBuyNow = () => {
    // Menggunakan env variable untuk nomor WA agar tidak di-hardcode di file produksi
    const waNumber = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "6281717302223"; 
  ...
  const deskripsiProduk = product.deskripsi || `Koleksi eksklusif ${product.nama_produk} dari Qodha Aromatic. Diracik menggunakan bahan-bahan alami pilihan dengan standar higienis yang tinggi. Menghasilkan aroma khas Timur Tengah yang tahan lama.`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 md:mt-8">
        <div className="bg-white rounded-4xl md:rounded-[3rem] shadow-xl border border-gray-100 overflow-hidden relative">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 relative">
            ...
            {/* Mengganti kalkulasi window scroll lama dengan Tailwind CSS Sticky Murni (Sangat Ringan bagi CPU) */}
            <div className="relative p-6 md:p-12 lg:p-16 bg-gray-50 flex items-center justify-center border-b lg:border-b-0 lg:border-r border-gray-100 lg:sticky lg:top-36 lg:h-[calc(100vh-160px)]">
              {isPremium && (
                <div className="absolute top-6 left-6 md:top-8 md:left-8 z-10 bg-gray-900 text-brand-gold border border-brand-gold/30 text-[10px] md:text-xs font-black px-4 py-2 rounded-full uppercase tracking-widest shadow-xl flex items-center gap-2">
